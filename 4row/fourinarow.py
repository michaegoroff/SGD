...
def runGame(isFirstGame):
    ...
    while True: # main game loop
        if turn == HUMAN:
            # Human player's turn.
            getHumanMove(mainBoard, showHelp, turn)
            if showHelp:
            ...
            animateComputerMoving(mainBoard, turn, column)
...
def drawBoard(board, turn, extraToken=None):
...
def getHumanMove(board, isFirstMove, turn):
    draggingToken = False
    ...
    while True:
            ...
            if event.type == QUIT:
                ...
                if tokeny < YMARGIN and tokenx > XMARGIN and tokenx < WINDOWWIDTH - XMARGIN:
                    ...
                    if isValidMove(board, column):
                        animateDroppingToken(board, column, RED, turn)
        ...
        if tokenx != None and tokeny != None:
            drawBoard(board, turn, {'x':tokenx - int(SPACESIZE / 2), 'y':tokeny - int(SPACESIZE / 2), 'color':RED})
        else:
            drawBoard(board, turn)
...
def animateDroppingToken(board, column, color, turn):
    ...
    while True:
        ...
        if int((y - YMARGIN) / SPACESIZE) >= lowestEmptySpace:
            return
        drawBoard(board, turn, {'x':x, 'y':y, 'color':color})
        pygame.display.update()
        FPSCLOCK.tick()


def animateComputerMoving(board, turn, column):
    ...
    while y > (YMARGIN - SPACESIZE):
        y -= int(speed)
        speed += 0.5
        drawBoard(board, turn, {'x':x, 'y':y, 'color':BLACK})
        pygame.display.update()
        FPSCLOCK.tick()
    ...
    animateDroppingToken(board, column, BLACK, turn)
...
def displayMove(role):
    FONT = pygame.font.Font('freesansbold.ttf', 16)
    if role == HUMAN:
        messageSurf = FONT.render('It\'s player\'s turn!', 1, RED)
    elif role == COMPUTER:
        messageSurf = FONT.render('It\'s computer\'s turn!', 1, BLACK)
    messageRect = messageSurf.get_rect()
    messageRect.topleft = (10, 10)
